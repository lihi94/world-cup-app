...
// === Random "monkey-style" prediction for users who forgot to submit ===
// Same weights as the Monkey bot — realistic football scores.
...
function randomPrediction() {
  return { scoreA: weightedScore(), scoreB: weightedScore() }
}
...
export async function runSync({ log = console.log } = {}) {
  ...
  // ===== Auto-fill missing predictions for newly-started matches =====
  // Rule: if a human player didn't submit a prediction, generate a random
  // monkey-style one so they at least get a chance at points.
  // (Bots are excluded — they have their own dedicated bot script.)
  let autoFilled = 0
  if (newlyStarted.length > 0) {
    const humans = await sb('/profiles?is_bot=eq.false&select=id')
    const humanIds = new Set(humans.map(h => h.id))

    const matchIds = newlyStarted.map(m => m.db_id)
    const existingPreds = await sb(
      `/predictions?match_id=in.(${matchIds.join(',')})&user_id=in.(${[...humanIds].join(',')})&select=user_id,match_id`
    )
    const hasPred = new Set(existingPreds.map(p => `${p.user_id}|${p.match_id}`))

    const toFill = []
    for (const m of newlyStarted) {
      for (const userId of humanIds) {
        if (hasPred.has(`${userId}|${m.db_id}`)) continue
        const { scoreA, scoreB } = randomPrediction()
        // For knockouts, also pick a random qualifier
        const isKnockout = m.stage !== 'GROUP'
        const predQualifierId = isKnockout
    ...
    if (toFill.length > 0) {
      ...
      for (let i = 0; i < toFill.length; i += BATCH) {
        await sb('/predictions', {
          method: 'POST',
          body: JSON.stringify(toFill.slice(i, i + BATCH)),
        })
      }
      autoFilled = toFill.length
      log(`🎲 Auto-filled ${autoFilled} random predictions for ${humanIds.size} players across ${newlyStarted.length} match(es)`)
    }
  }
