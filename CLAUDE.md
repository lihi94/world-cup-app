...
|----------|---------|--------|
| `fetch-results` | v18 | מעדכן תוצאות כל **5 דקות** (pg_cron job id=2) |
...
- **תיוג נוקאאוט לפי חלונות-תאריך (v18, בלוק 2.5)**: football-data מתייג את כל הטורניר `GROUP_STAGE` עד שהבראקט נמשך רשמית, אז משחקי R32/R16 יושבים מתויגים GROUP בטעות (גם בראקט שגוי וגם ניקוד שגוי — נוקאאוט הוא 4/3 + בונוס, לא 3/2). הבלוק מתייג כל משחק GROUP שנופל בחלון **R32** (28/6 07:00Z – 4/7 06:59Z) או **R16** (4/7 07:00Z – 9/7 06:59Z) — חלונות קבועים שנלקחו מלוח ESPN (`leagues[0].calendar`). רץ **אחרי** football-data (דורס את ה-GROUP שהוא כתב) ו**לפני** הניקוד (שהנקודות יהיו נכונות). idempotent (נוגע רק בשורות GROUP). QF/SF/THIRD/FINAL לא נגעים (football-data מתייג אותם נכון). **הערה:** ניסיון ראשון (v17) קרא את הלוח דינמית וכשל (שם השדה בלוח שונה מהצפוי), לכן עברנו לחלונות קבועים. אומת אחרי ריצת פונקציה מלאה: 72 GROUP / 16 R32 / 8 R16 / 4 QF / 2 SF / 1 THIRD / 1 FINAL = 104.
